...
rsvp_status_enum      = postgresql.ENUM("accepted","declined","maybe","pending",        name="rsvp_status")
dish_category_enum    = postgresql.ENUM("starter","main","side","dessert","drink","other", name="dish_category")
budget_entry_type_enum = postgresql.ENUM("host","participant",                          name="budget_entry_type")
...
rsvp_status_col       = postgresql.ENUM("accepted","declined","maybe","pending",           name="rsvp_status",       create_type=False)
dish_category_col     = postgresql.ENUM("starter","main","side","dessert","drink","other", name="dish_category",     create_type=False)
budget_entry_type_col = postgresql.ENUM("host","participant",                              name="budget_entry_type", create_type=False)
reminder_type_col     = postgresql.ENUM("push","email","both",                             name="reminder_type",     create_type=False)
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("guest_count_estimate", sa.Integer,     nullable=False, server_default="0"),
        ...
        sa.Column("show_shopping_list",   sa.Boolean,     nullable=False, server_default="false"),
        sa.Column("show_prep_tasks",      sa.Boolean,     nullable=False, server_default="false"),
    ...
    op.create_index("ix_gatherings_invite_token", "gatherings", ["invite_token"], unique=True)

    # ── 3. guest_rsvps — uses rsvp_status_col (create_type=False) ─────────
    op.create_table(
        "guest_rsvps",
        sa.Column("id",           UUID(as_uuid=True), primary_key=True, server_default=sa.text("gen_random_uuid()"), nullable=False),
        sa.Column("gathering_id", UUID(as_uuid=True), sa.ForeignKey("gatherings.id", ondelete="CASCADE"), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column("name",             sa.String(255), nullable=False),
        sa.Column("category",         dish_category_col, nullable=False, server_default="other"),
        sa.Column("assigned_to",      sa.String(255), nullable=True),
        sa.Column("is_host_prepared", sa.Boolean,     nullable=False, server_default="true"),
    ...
    op.create_table(
        ...
        sa.Column("id",                UUID(as_uuid=True), primary_key=True, server_default=sa.text("gen_random_uuid()"), nullable=False),
        sa.Column("gathering_id",      UUID(as_uuid=True), sa.ForeignKey("gatherings.id", ondelete="CASCADE"), nullable=False),
        sa.Column("dish_id",           UUID(as_uuid=True), sa.ForeignKey("dishes.id",     ondelete="SET NULL"), nullable=True),
        sa.Column("name",              sa.String(255),   nullable=False),
    ...
    op.create_index("ix_shopping_items_gathering_id", "shopping_items", ["gathering_id"])
    op.create_index("ix_shopping_items_dish_id",      "shopping_items", ["dish_id"])
    op.create_index("ix_shopping_items_is_purchased", "shopping_items", ["is_purchased"])
    ...
    op.create_table(
        ...
        sa.Column("reminder_type", reminder_type_col, nullable=False, server_default="push"),
    ...
    for table in [
        "users", "gatherings", "guest_rsvps", "menus", "dishes",
        "recipes", "shopping_items", "prep_tasks",
        "budgets", "budget_entries", "reminders",
    ]:
        op.execute(f"""
...
def downgrade() -> None:
    tables = [
        "reminders", "budget_entries", "budgets", "prep_tasks",
        "shopping_items", "recipes", "dishes", "menus",
        "guest_rsvps", "gatherings", "users",
    ]
    bind = op.get_bind()
    for table in tables:
